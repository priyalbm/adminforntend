...
export const fetchAllBot = createAsyncThunk(
  ...
  async ({ page, limit, searchQuery }, { rejectWithValue, getState }) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/bot-configs/?page=${page}&page_size=${limit}&search=${searchQuery}`, {
        headers: {
...
export const botStatusUpdate = createAsyncThunk(
  ...
  async ({id,status}, { rejectWithValue }) => {
    try {
      const response = await fetch(`${baseUrl}/api/bot-configs/${id}/${status}/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      const data = await response.json();
      if (!response.ok) {
        return rejectWithValue(data.error || data.detail || `bot ${status} successfuly`);
      }
      return data;
    } catch (error) {
